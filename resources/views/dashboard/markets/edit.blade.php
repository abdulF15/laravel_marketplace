                ...
                <form class="space-y-4 md:space-y-6" action="/dashboard/market/{{ $market->id }}" method="post">
                    ...
                    <div>
                        ...
                        @foreach ($categories as $category)
                            @if (old('category_id',$market->category_id) == $category->id)
                    ...
                    <div>
                        ...
                        <input required type="number" name="price" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-green-600 focus:border-green-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('price') border-red-500  @enderror" placeholder="$" required value="{{ old('price',$market->price) }}">
                    </div>
                    <button type="submit" class="w-full text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Edit Product</button>
                </form>
            </div>
        </div>
    </div>
